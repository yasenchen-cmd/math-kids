...
export default function PayWall({ onCreateOrder, onStartPolling }) {
  ...
  const handlePay = async () => {
    ...
    try {
      ...
      setOrder(data)
      if (data.order_id) onStartPolling(data.order_id, () => {})
    } catch (e) {
      ...
      <div style={card}>
        ...
        {!order ? (
          <>
            ...
            <button
              style={{ ...payBtn, opacity: loading ? 0.6 : 1, cursor: loading ? 'wait' : 'pointer' }}
            ...
            {order.qr_url ? (
              ...
              <div style={demoQr}>
                ...
                <a href={order.order_url || '#'} target="_blank" rel="noopener noreferrer"
                  style={{ color: '#4F8CF6', fontSize: '0.8rem' }}>
                  点击模拟支付
                </a>
              </div>
            )}
...
const demoQr = {
  background: '#F5F5F5', borderRadius: '16px', padding: '24px', marginBottom: '12px',
}
const wechatBox = {
  background: '#fff', border: '2px solid #D4380D', borderRadius: '10px',
  padding: '8px 12px', marginTop: '8px', fontSize: '1rem',
  fontWeight: 700, color: '#D4380D', userSelect: 'all',
  cursor: 'pointer',
}
